...
int main(int argc, char **argv)
{
  ...
  const auto mesh = Mesh { std::move(grid.nodes), std::move(grid.simplices), {hpm.GetL1PartitionNumber(), hpm.GetL2PartitionNumber()}, hpm.gaspi_runtime.rank().get(), partitioner };
  ...
  std::stringstream ss;
  ss << "L1 partitions = " << hpm.GetL1PartitionNumber() << ", L2 partitions = " << hpm.GetL2PartitionNumber() << ", my rank = " << hpm.gaspi_runtime.rank().get() << "\n";
  ...
  HPM::DistributedDispatcher dispatcher{hpm.gaspi_context, hpm.gaspi_segment, hpm};
